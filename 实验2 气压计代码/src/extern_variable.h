...
extern uint8 accUpdated;

//系统
extern uint8 SystemReady_OK;                    // 系统初始化完成标志
extern uint8 FlyEnable;					

//飞行开关
extern uint8 IIC_ERROR_CNT;                     // iic错误计数器,每次tim3中断加1,如果读取成功置0
extern uint8 I2C2_BUSY;
volatile extern uint32 TIM3_IRQCNT;             // TIM3中断计数器
volatile extern uint32 TIM2_IRQCNT;             // TIM2中断计数器
volatile extern uint8 MPU6050_I2CData_Ready;    // mpu6050读取完成标志,=1表示读取完成
