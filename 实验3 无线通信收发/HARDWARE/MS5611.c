...
void MS561101BA_readPROM(void) 
{
	...
	for (i=0;i<MS561101BA_PROM_REG_COUNT;i++) 
	{
		...
		#else
			I2c_Start();
			I2c_SendByte(MS5611_ADDR);
			I2c_WaitAck();
			I2c_SendByte(MS561101BA_PROM_BASE_ADDR + (i * MS561101BA_PROM_REG_SIZE));
			I2c_WaitAck();	
			I2c_Stop();
			delay_us(5);
			I2c_Start();
			I2c_SendByte(MS5611_ADDR+1);  //进入接收模式	
			delay_us(1);
			I2c_WaitAck();
			inth = I2c_ReadByte();  //带ACK的读数据
		   I2c_Ack();
			delay_us(1);
			intl = I2c_ReadByte();	 //最后一个字节NACK
	   	I2c_NoAck();
			I2c_Stop();
...
void MS561101BA_reset(void) 
{
	...
	#else
	I2c_Start();
    I2c_SendByte(MS5611_ADDR); //写地址
	I2c_WaitAck();
    I2c_SendByte(MS561101BA_RESET);//发送复位命令
	I2c_WaitAck();	
    I2c_Stop();
	#endif
}
...
uint32_t MS561101BA_getConversion(void) 
{
	...
	I2c_SendByte(MS5611_ADDR+1);  //进入接收模式	
	I2c_WaitAck();
	temp[0] = I2c_ReadByte();  //带ACK的读数据  bit 23-16
	 I2c_Ack();
	temp[1] = I2c_ReadByte();  //带ACK的读数据  bit 8-15
	 I2c_Ack();
	temp[2] = I2c_ReadByte();  //带NACK的读数据 bit 0-7
		I2c_NoAck();
	I2c_Stop();
...
float MS561101BA_get_altitude(void)
{
	...
	Altitude = 
	4433000.0 * (1 - pow((MS5611_Pressure / Alt_offset_Pa), 0.1903))*0.01f;
