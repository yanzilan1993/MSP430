...
int8_t MpuInit(void) //初始化
{
    ...
    do
    {
        date = IICwriteByte(MPU6050_ADDRESS, PWR_MGMT_1, 0x80);	  //复位
        delay_ms(30);
        date += IICwriteByte(MPU6050_ADDRESS, SMPLRT_DIV, 0x02);  //陀螺仪采样率，0x00(333Hz)
        date += IICwriteByte(MPU6050_ADDRESS, PWR_MGMT_1, 0x03);  //设置设备时钟源，陀螺仪Z轴
        date += IICwriteByte(MPU6050_ADDRESS, CONFIGL, 0x03);     //低通滤波频率，0x03(42Hz)
...
#define  Acc_Read() IICreadBytes(MPU6050_ADDRESS, 0X3B,6,buffer) //读取加速度
#define  Gyro_Read() IICreadBytes(MPU6050_ADDRESS, 0x43,6,&buffer[6])  //  读取角速度
