...
void IMUSO3Thread(void)
{
    //! Time constant
    float dt = 0.01f;		//s
    static uint64 tPrev=0, startTime=0;	//us
    uint64 now;
    uint8 i;
    ...
    if(!imu.ready)//如果imu没有准备好，则需要进行陀螺仪补偿，时间为3s
    {
        if(startTime==0)
        {
            startTime = now;//获取开始时间
        }
        ...
        {
            imu.gyrOffset[0] = gyro_offsets_sum[0]/offset_count;
            imu.gyrOffset[1] = gyro_offsets_sum[1]/offset_count;
            imu.gyrOffset[2] = gyro_offsets_sum[2]/offset_count;
    ...
    gyro[0] = imu.gyro[0] - imu.gyrOffset[0];
    gyro[1] = imu.gyro[1] - imu.gyrOffset[1];
    gyro[2] = imu.gyro[2] - imu.gyrOffset[2];

    acc[0] = -imu.accb[0];
    acc[1] = -imu.accb[1];
    acc[2] = -imu.accb[2];

    // NOTE : Accelerometer is reversed.加速度计是颠倒的
    // Because proper mount of PX4 will give you a reversed accelerometer readings.
    // CrazyPony的核心算法，用来验算姿态 
    ...
    // 角度
    imu.rol_deg = euler[0] * 180.0f / M_PI_F;
    imu.pit_deg = euler[1] * 180.0f / M_PI_F;
    imu.yaw_deg = euler[2] * 180.0f / M_PI_F;
    
    DbgInfo.imu_rol = imu.rol_deg;
    DbgInfo.imu_pit = imu.pit_deg;
    DbgInfo.imu_yaw = imu.yaw_deg;
}
